					...
					for zipped_file in myzip.namelist():
						if zipped_file[-4:] == '.xml':
							...
							with myzip.open(zipped_file, "r") as xml_file:
								...
								# print(code)
								type_object = object_code[code]
								if type_object == 'Земельный участок':
									cad_number = root.find('.//cad_number').text
									print(cad_number)
									# write_log(f'----{cad_number}----')
									# change_status('Скачан', now, cad_number)
									readable_address = root.find('.//readable_address').text
									print(readable_address)
									included_objects = root.findall('./land_record/cad_links/included_objects/')
									for i in included_objects:
										included_cad_number = i.find('./cad_number').text
										print(f'included_cad_number: {included_cad_number}')
										write_log(included_cad_number)
